...
import { Building2, User, Lock, ChevronRight, AlertCircle } from 'lucide-react';
...
export const LoginPage = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    const user = await dataService.login(userId, password);
    if (user) navigate('/dashboard');
    else setError('Invalid Credentials or Account Not Found');
  };
    ...
    <div className="min-h-screen bg-slate-50 flex items-center justify-center p-6">
      <div className="max-w-md w-full bg-white rounded-3xl shadow-2xl p-10 relative overflow-hidden">
        ...
        <div className="text-center mb-10">
          <div className="inline-flex items-center justify-center bg-indigo-50 p-4 rounded-2xl text-indigo-600 mb-4">
            <Building2 size={32} />
          </div>
          <h1 className="text-2xl font-black text-slate-900">{company?.name}</h1>
          <p className="text-slate-500 font-medium">Inventory Access Terminal</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          {error && (
            <div className="flex items-center gap-2 p-3 rounded-xl bg-rose-50 text-rose-600 text-sm font-bold animate-in fade-in slide-in-from-top-1">
              <AlertCircle size={16} />
              <span>{error}</span>
            </div>
          )}

          <button type="submit" className="w-full bg-slate-900 text-white py-4 rounded-2xl font-black text-lg flex items-center justify-center space-x-2 hover:bg-indigo-600 transition-all shadow-xl shadow-slate-200">
            <span>Log In</span>
